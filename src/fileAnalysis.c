...
FileArray *getSplitFileSizes(char filename[150]) {
    ...
    FileArray *splitFS = (FileArray *) malloc(1 * sizeof(File));
    ...
    splitFS->files = (File *) malloc(sizeof(File) * 5);
    splitFS->capacity = 5;
    splitFS->size = 0;
    ...
    for (int i = 0; MainFileSize > 0; ++i) {
        ...
        splitFS->files[i].FileSize = MinFileSize;
